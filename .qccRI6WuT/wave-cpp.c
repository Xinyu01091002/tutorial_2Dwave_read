  ...
  if (!restore("restart"))
  {
    ...
    do
    {
      ...
      foreach ()
      {
        foreach_dimension()
        {
          u.x[] = (my_phi[1] - my_phi[-1]) / (2.0 * Delta*fm.x[]) * f[]/1.56;
 ...
 foreach ()
    ...
    not refine the mesh anymore. */
        ...
        while (adapt_wavelet({f, u},
                             (double[]){0.02, uemax, uemax/y_metric, uemax}, LEVEL, 5)
                   .nf);
    #else
        while (0);
    #endif
...
event adapt(i++)
{
  adapt_wavelet({f, u}, (double[]){0.01, uemax, uemax/y_metric, uemax}, LEVEL, 5);
}
#endif
